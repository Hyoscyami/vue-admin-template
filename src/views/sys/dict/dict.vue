...
import {useFilterTree, useGetList, useListStatus, useViewDetail} from '@/composables/sys/dict/dict'
...
export default {
  ...
  setup() {
    ...
    // 状态转换
    const viewDetailDataStatus = computed(() => { return dictConvert(DictEnum.DictStatus, this.dialog.viewDetailData.status) })
    // 日期转换
    const viewDetailDataCreateTime = computed(() => { return format(this.dialog.viewDetailData.createTime) })
    // 树
    const treeRef = ref(null)
    // 搜索权限树的时候联动过滤名称符合的树
    watch(() => tree.filterTreeText, () => { useFilterTree(tree, treeRef) })
    // total改变了 ，计算是否能继续滚动加载树
    watch(() => tree.total, () => { tree.scrollTreeDisable = tree.listQuery.page * tree.listQuery.limit >= tree.total })
    const {listStatus} = useListStatus(table)
    const {searchFormSubmit} = useGetList(table, tree)
    onMounted(() => {
      // 初始化状态
      listStatus
      // 初始化表格
    ...
    const {viewDetail} = useViewDetail(dialog, row)
  },
  methods: {
    resetForm(formName) {
